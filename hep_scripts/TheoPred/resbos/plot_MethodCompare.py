...
asym.GetYaxis().SetRangeUser(-0.02,0.02)##
# ------------------------------------------------------------------------------------------------------------------//
...
asym.Reset()   
for i, result in enumerate(W_MC) :
  asym.SetBinContent(i+1,result[0]-(W_MC[i])[0])
...
asym.SetMarkerStyle(20);
h0 = asym.Clone()
leg.AddEntry(h0,"Monte Carlo","l")##  
asym.DrawCopy("E1")  


asym.Reset()   
for i, result in enumerate(W_Sil) :
  asym.SetBinContent(i+1,result[0]-(W_MC[i])[0])
  asym.SetBinError(i+1,0.00000000000000000001)#result[1])
...
asym.SetMarkerStyle(20);
h1 = asym.Clone()
leg.AddEntry(h1,"Silvia's Corrections","l")##  
asym.DrawCopy("E1 same") 

asym.Reset()   
for i, result in enumerate(W_Metcorr) :
  asym.SetBinContent(i+1,result[0]-(W_MC[i])[0])
  asym.SetBinError(i+1,0.00000000000000000001)#result[1])
asym.SetLineColor(46)  
asym.SetLineWidth(2)
asym.SetMarkerStyle(20);
h2 = asym.Clone()
leg.AddEntry(h2,"MIT Corrected for Eta","l")##  
asym.DrawCopy("E1 same")  

asym.Reset()   
for i, result in enumerate(W_MET) :
  asym.SetBinContent(i+1,result[0]-(W_MC[i])[0])
  asym.SetBinError(i+1,0.00000000000000000001)#result[1])
asym.SetLineColor(24)  
asym.SetLineWidth(2)
asym.SetMarkerStyle(20);
h2 = asym.Clone()
leg.AddEntry(h2,"MIT ntuple","l")##  
asym.DrawCopy("E1 same")  
