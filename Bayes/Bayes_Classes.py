...
class BayesData(dict):
    ...
    def __init__(self, name='', pool=None, response=""):
        self.name = name
        self.training = []
        self.pool = pool
        self.tokenCount = 0
...
class TimeBayesData(BayesData):
    def respond(self):
        tmp = self.response + str(datetime.time)
        return tmp
...
class myBayes(Bayes):
    def getTokens(self, obj):
        """Edit this to remove caps, split ? and ! and , and . away from words
        """
        return self._tokenizer.tokenize(obj)
